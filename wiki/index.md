...
- [[query工作流]]：基于已编译 Wiki 进行问答并反向沉淀高质量答案的工作流：index 定位、读 Wiki 生成结论先行答案、有长期复用价值时以 source_summary + query_write 标签回写；wiki 未命中时 L1 wiki / L2 raw / L3 联网三路并行检索，按来源优先级 外链 > wiki > raw > 项目路径 取舍，L3 联网结果不落盘 raw。
