...
> 铁律：`raw/` 目录AI **只读 + 只增不改**——可读取、可新增素材文件（如联网抓取的网页、整理落盘的候选素材），**禁止修改、删除、覆盖任何已有素材**，以保事实基准与溯源依据不被污染。
...
6. **冲突检测与调和**：遇到信息冲突禁止直接覆盖旧内容。Agent 先自行调和：核对原始素材、区分「视角补充」与「语义矛盾」，可调和则合并更新并附【当前采信结论】及依据，同时把处理结论告知用户；仅当证据不足、无法调和时，才标记 `⚠️观点冲突` 并列双方依据来源，冲突页面暂停自动更新，进入人工裁决队列，由人工在步骤8验收环节一并处理。
...
3. **生成答案**：
   ...
   - 不同页面存在观点冲突时，Agent 先自行调和：核对依据、可调和则附【当前采信结论】并告知用户；无法调和才并列双方表述与依据标记「⚠️观点冲突」交人工裁决。
...
7. **缺口回退（wiki 未命中时）**：按「L1 本地 wiki → L2 raw/本地项目其他信息（并行）→ L3 联网 → 兜底」逐层下探，命中即停：
   - 命中层标注来源：wiki 双链 > raw 双链 > 本地项目路径 > 互联网外链；
   - 命中内容过「长期复用价值」判定：有价值 → Ingest + 人工抽检验收入库；无价值 → 仅本次对话作答并标注来源；
   - 判定有价值但内容与 wiki 已有结论语义冲突 → Agent 先自行调和：核对依据、可调和则更新页面并附【当前采信结论】且告知用户；无法调和才标记 `⚠️观点冲突` 保留双方原文交人工裁决，AI 不自动覆盖；
   - L3 联网抓取的网页一律视为 raw 原始素材，存入 `raw/articles/`（只增不改），禁止直接写入 wiki、禁止粘贴摘要；
...
3. **观点冲突**：Agent 先核对原始素材，补充【当前采信结论】并附依据、告知用户；无法调和才保留双方历史表述标记冲突交人工裁决。
...
### 3.4 可选扩展：Agent 联网搜索能力

> 本能力非基础必需，依赖 Agent 具备联网工具；可随时关闭该能力。

**核心规则**

1. 查询优先级：本地 Wiki 优先；wiki 信息充足，**禁止**调用联网搜索。
2. 触发联网条件：wiki 内容缺失、论据不足、知识已过时。
3. 联网获取的网页属于 **raw 原始素材**，由 AI 存入 `raw/articles/`（只增不改）；**禁止直接写入 wiki、禁止粘贴搜索摘要**。
4. 价值判断：
   - 临时一次性信息：仅对话输出，标注【来源：互联网搜索】，不入库；
   - 长期复用知识：网页作为 raw 素材，执行标准 Ingest 全流程，人工抽检验收后入库。
5. 冲突处理：与现有 wiki 页面语义冲突时，Agent 先自行调和（核对原始素材、判断采信结论并附依据，处理结果告知用户）；无法调和才标记 `⚠️观点冲突：[[xxx]]`，保留双方原文，进入人工裁决队列，AI 不自动覆盖已有结论。
6. 溯源：Ingest 生成 wiki 页面后，`source` 双链指向 raw 下的网页素材文件，保留网络来源信息。

**风险提示**

1. 互联网存在虚假、过时、片面信息，网络素材与其他 raw 素材同等对待，**必须经过人工验收，不能信任搜索结果**。
2. 严禁 Agent 绕过 Ingest 直接把搜索内容粘贴进 wiki，会污染知识库形成[[结构化垃圾场]]。
3. 联网搜索得到的素材同样参与 shturl.md 日志记录。
...
- **自判断配图**：生成 Wiki 页面时，AI 需自行判断内容是否需要配图——凡属**流程图、架构组织、机制机制、建设路线**等可图示化内容，必须配 1 张 Mermaid 图；纯表格对比、简单事实列举等无需配图时可不配。
- **必须配图的页面类型**：
  - 工作流类（concept）：Ingest / Lint / Query 各配「流程总览」图；
  - 架构组织类：[[三层架构]]、[[LLM-Wiki框架]] 配架构结构图；
  - 机制类：[[防膨胀闸门]] 配三闸门流程图；
  - 路线类：overview 建设方案配分阶段路线图；
  - 概念对比类（可配）：[[编译型知识库]] 等可用对比示意图辅助。
- **绘制规范**：一律使用 Mermaid（` ```mermaid ` 代码块），禁止 ASCII 字符画；节点标签含特殊字符时用双引号包裹（如 `A["Add & Norm"]`）；无法用 Mermaid 表达时才允许纯文字描述。图放置于页面标题下方、`核心结论` 之前（如「流程总览」「架构图」小节）。
- **时效**：新增/修改页面时随正文一并配图，`updated_at` 刷新。
