...
7. **缺口回退（wiki 未命中时）**：L1 本地 wiki、L2 raw/本地项目信息、L3 联网**三路并行检索**，结果汇总后按来源优先级取舍：**互联网外链 > wiki 双链 > raw 双链 > 本地项目路径**（wiki 非实时更新，联网权威信息优先采信）：
   - 命中内容过「长期复用价值」判定：有价值 → 登记缺口看板 `raw/_pending/`，由用户决定是否另走抓取入库流程；无价值 → 仅本次对话作答并标注来源；
   - 判定有价值但内容与 wiki 已有结论语义冲突 → Agent 先自行调和：核对依据、可调和则按来源优先级裁决并附【当前采信结论】且告知用户；无法调和才标记 `⚠️观点冲突` 保留双方原文交人工裁决，AI 不自动覆盖；
   - L3 联网结果**不落盘 raw**：仅本次对话参考并标注【来源：互联网搜索】，禁止直接写入 wiki、禁止粘贴摘要；
...
1. 查询模式：L1 本地 wiki、L2 raw/本地项目信息、L3 联网**三路并行触发**，结果按来源优先级（互联网外链 > wiki > raw > 项目路径）取舍；wiki 充足时也允许联网核验时效性。
2. 触发联网条件：wiki 内容缺失、论据不足、知识已过时，或需核验 wiki 结论时效性。
3. 联网结果**不落盘 raw**：仅作为本次对话参考并标注【来源：互联网搜索】，引擎**禁止将网页存入 `raw/articles/`**、禁止直接写入 wiki、禁止粘贴搜索摘要。
4. 价值判断：
   - 临时一次性信息：仅对话输出，标注【来源：互联网搜索】，不入库；
   - 长期复用知识：登记缺口看板 `raw/_pending/`，由用户决定是否另行抓取落盘后执行 Ingest。
5. 冲突处理：与现有 wiki 页面语义冲突时，参考来源优先级（互联网外链 > wiki 双链 > raw 双链 > 本地项目路径）综合裁决，Agent 先自行调和（核对原始素材、判断采信结论并附依据，处理结果告知用户）；无法调和才标记 `⚠️观点冲突：[[xxx]]`，保留双方原文，进入人工裁决队列，AI 不自动覆盖已有结论。
6. 溯源：用户确认抓取落盘并经 Ingest 生成 wiki 页面后，`source` 双链指向 raw 下的素材文件，保留网络来源信息。

**风险提示**

1. 互联网存在虚假、过时、片面信息，网络信息**必须经过人工验收，不能信任搜索结果**。
2. 严禁 Agent 绕过 Ingest 直接把搜索内容粘贴进 wiki，会污染知识库形成[[结构化垃圾场]]。
3. 联网结果不落盘 raw，**不写入 shturl.md 日志**；仅当用户确认抓取落盘后再按 Ingest 流程记录。

### 3.5 Agent 开发学习参考资源

> 用户主要学习方向为 **Agent（智能体）开发**。以下两个链接是 Agent / AI 工程化领域常用的学习与取材来源，涉及该领域知识缺口或素材抓取时可参考：

1. **GitHub 仓库**：https://github.com/rohitg00/ai-engineering-from-scratch.git —— AI 工程化从零实现系列，含 Agent 构建源码与教程
2. **中文文档站**：https://aiengineeringfromscratch.docpage.cn/ —— 同一内容的中文可读站点

**使用要点**
- 日常浏览、提取素材读文档站即可；核对源码细节、原始表述时访问 GitHub。
- 两站表述不一致时以 GitHub 原仓库为参考。
- **纪律不变**：联网素材仍存 `raw/`（只增不改）；长期复用走 Ingest + 人工验收，临时查询只标注【来源：互联网搜索】，禁止直接写入 wiki。
