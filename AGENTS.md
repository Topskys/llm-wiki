...
- 当用户说「抓取某个链接内容」时，默认为完整编译流程：**① 抓取提取原文 → ② 联网检索验证/补充/融合 → ③ 输出融合论文 markdown 到 `raw/papers/`**，而不是仅提取素材或仅落盘 `raw/articles/`。
- 该流程同样适用于后续可选的全库编译入库（新增/联动更新 wiki 页面 → 同步 index.md 与 shturl.md）。

## 论文格式约定（raw/papers/）

- 标题**不宜过长**（尽量 ≤20 字，避免长副标题堆砌）。
- 摘要500字以内，关键词在摘要下边，格式`**关键词**：…` 提炼 10 个以内关键词。
- **禁止出现「一句话总结」字样**。
- 参考文献区**每条引用独立成段（条目间须有空行换行）**；格式沿用：锚点 + 编号 + 可点击标题超链接。

## 缺口回退与联网搜索约束

Query 时 wiki 未命中，L1 本地 wiki、L2 raw/本地项目信息、L3 联网**三路并行检索**，汇总后按来源优先级取舍：**互联网外链 > wiki > raw > 项目路径**（wiki 非实时更新，联网权威信息优先采信）：

1. **并行触发**：三路同时检索；wiki 信息充足参考上一步优先级，不再「必须先用 wiki、充足严禁联网」。
2. **L3 不落盘**：联网结果仅本次对话参考，标注【来源：互联网搜索】；**禁止存入 `raw/articles/`**、禁止直接写入 wiki、禁止粘贴摘要。
3. **价值判定**：长期复用 → 登记缺口看板 `raw/_pending/`，由用户决定是否另行抓取落盘入库；临时一次性查询仅对话输出，标注【来源：互联网搜索】，不入库。
4. **冲突**：内容与 wiki 已有结论语义冲突时，参考来源优先级综合裁决，Agent 先自行调和（核对素材、附【当前采信结论】并告知用户）；无法调和才标 `⚠️观点冲突` 交人工裁决，不自动覆盖。
5. **兜底**：组合推理（标【推导】）→ 反问用户补料 → 登记缺口看板 `raw/_pending/`，素材到位后触发 Ingest。

## 参考资源（Agent 学习）

用户主要学习 **Agent 开发**；Agent / AI 工程化领域的知识缺口与取材，可参考这两个链接（详见 SCHEMA.md 3.5）：

- **GitHub 仓库**：https://github.com/rohitg00/ai-engineering-from-scratch.git —— AI 工程化从零实现，含 Agent 构建源码与教程
- **中文文档站**：https://aiengineeringfromscratch.docpage.cn/ —— 同一内容的中文可读站点
